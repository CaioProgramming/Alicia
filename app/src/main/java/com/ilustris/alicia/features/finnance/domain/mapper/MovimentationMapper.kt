...
class MovimentationMapper {
    ...
    fun mapMovimentations(movimentations: List<Movimentation>): List<MovimentationInfo> {
        ...
        val movimentationGroups = movimentations.groupBy { it.tag.findTag() }
    ...
    fun mapMovimentationsByDay(movimentations: List<Movimentation>): List<MovimentationInfo> {
        ...
        val movimentationGroups =
            movimentations.sortedByDescending { it.spendAt }.groupBy {
                val calendar =
    ...
    fun mapMovimentationsToLineData(movimentations: List<Movimentation>): List<LineData> {
        val movimentationsGroup =
            mapMovimentationsByDay(movimentations).reversed().map {
    ...
    fun mapMovimentationsToCircleData(movimentations: List<Movimentation>) =
        mapMovimentations(movimentations)
            .map { info ->
    ...
    fun mapGoalsToInfo(goals: List<Goal>) =
        goals.groupBy { it.tag.findTag() }.map {
            GoalInfo(
